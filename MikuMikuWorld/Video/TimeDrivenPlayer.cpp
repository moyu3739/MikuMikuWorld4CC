...
bool TimeDrivenPlayer::FrameReady(unsigned int backend_id) const{
    if (!fps_lock) return true;
    else return frame_ready & (1u << backend_id);
}
